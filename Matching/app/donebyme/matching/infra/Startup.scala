...
import donebyme.matching.infra._
import donebyme.matching.views._
...
class StartupRunner @Inject() (system: ActorSystem, ws: WSClient) extends Startup {
  ...
  val properties = Map[String,String](
    ("home" -> 		s"http://${host}:${port}/matching"),
    ("submitProposal" ->	s"http://${host}:${port}/matching/proposals"),
    ("proposal" ->		s"http://${host}:${port}/matching/proposals/:id")
  )
